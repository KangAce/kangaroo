...
import ink.kangaroo.system.domain.SysConfig;
import ink.kangaroo.system.service.ISysConfigService;
...
public class SysConfigController extends BaseController {
    ...
    public AjaxResult getInfo(@PathVariable Long configId) {
        return AjaxResult.success().setData(configService.selectConfigById(configId));
    }
    ...
    public AjaxResult add(@Validated @RequestBody SysConfig config) {
        if (UserConstants.NOT_UNIQUE.equals(configService.checkConfigKeyUnique(config))) {
            return AjaxResult.fail("新增参数'" + config.getConfigName() + "'失败，参数键名已存在");
        }
    ...
    public AjaxResult edit(@Validated @RequestBody SysConfig config) {
        if (UserConstants.NOT_UNIQUE.equals(configService.checkConfigKeyUnique(config))) {
            return AjaxResult.fail("修改参数'" + config.getConfigName() + "'失败，参数键名已存在");
        }
