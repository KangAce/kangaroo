...
import ink.kangaroo.system.api.domain.SysDept;
import ink.kangaroo.system.service.ISysDeptService;
...
//    @RequiresPermissions("system:dept:query")
    ...
    public AjaxResult getInfo(@PathVariable Long deptId) {
        ...
        return AjaxResult.success().setData(deptService.selectDeptById(deptId));
    }
    ...
    public AjaxResult treeselect(SysDept dept) {
        List<SysDept> depts = deptService.selectDeptList(dept);
        return AjaxResult.success().setData(deptService.buildDeptTreeSelect(depts));
    }
...
//    @RequiresPermissions("system:dept:add")
    ...
    public AjaxResult add(@Validated @RequestBody SysDept dept) {
        if (UserConstants.NOT_UNIQUE.equals(deptService.checkDeptNameUnique(dept))) {
            return AjaxResult.fail("新增部门'" + dept.getDeptName() + "'失败，部门名称已存在");
        }
...
//    @RequiresPermissions("system:dept:edit")
    ...
    public AjaxResult edit(@Validated @RequestBody SysDept dept) {
        if (UserConstants.NOT_UNIQUE.equals(deptService.checkDeptNameUnique(dept))) {
            return AjaxResult.fail("修改部门'" + dept.getDeptName() + "'失败，部门名称已存在");
        } else if (dept.getParentId().equals(dept.getDeptId())) {
            return AjaxResult.fail("修改部门'" + dept.getDeptName() + "'失败，上级部门不能是自己");
