package fun.imore.tribe.service.impl;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import fun.imore.tribe.cache.AbstractStringCacheStore;
import fun.imore.tribe.config.properties.TribeProperties;
import fun.imore.tribe.model.entity.Attachment;
import fun.imore.tribe.model.entity.PixivLike;
import fun.imore.tribe.model.entity.pixiv.PixivArtword;
import fun.imore.tribe.model.entity.pixiv.PixivRank;
import fun.imore.tribe.model.entity.pixiv.PixivRankArtword;
import fun.imore.tribe.model.support.TribeConst;
import fun.imore.tribe.model.vo.PixivArtwordDetailVo;
import fun.imore.tribe.model.vo.PixivImageUrlVo;
import fun.imore.tribe.repository.PixivLikeRepository;
import fun.imore.tribe.repository.pixiv.ArtworkRepository;
import fun.imore.tribe.repository.pixiv.PixivArtwordRepository;
import fun.imore.tribe.repository.pixiv.PixivRankArtwordRepository;
import fun.imore.tribe.repository.pixiv.PixivRankRepository;
import fun.imore.tribe.service.AttachmentService;
import fun.imore.tribe.service.OptionService;
...
import ink.kangaroo.pixiv.repository.pixiv.ArtworkRepository;
import ink.kangaroo.pixiv.repository.pixiv.PixivArtwordRepository;
import ink.kangaroo.pixiv.repository.pixiv.PixivRankArtwordRepository;
import ink.kangaroo.pixiv.repository.pixiv.PixivRankRepository;
...
//        PixivArtword byIllustId = pixivArtwordRepository.findByIllustId(id);
    ...
    private Attachment getAttachment(String filePathUrl) {
        ...
        try {
            //判断文件是否已经在下载
            String s = cacheStore.get(fileFullName).orElse(null);
            while ("locked".equals(s)) {
                log.info("已经有进程正在下载：" + fileFullName);
                //是否已经解锁循环获取 或者直接报错？
                try {
                    Thread.sleep(2000);
                    s = cacheStore.get(fileFullName).orElse(null);
...
//                upload = (JSONObject) JSON.parse(s);
                ...
                cacheStore.put(fileFullName, "locked", 5, TimeUnit.MINUTES);
            ...
            cacheStore.put(fileFullName, JSON.toJSONString(upload), 5, TimeUnit.MINUTES);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            //TODO
            if ("locked".equals(cacheStore.get(fileFullName))) {
                cacheStore.delete(fileFullName);
            }
        }
        return upload;
    }
