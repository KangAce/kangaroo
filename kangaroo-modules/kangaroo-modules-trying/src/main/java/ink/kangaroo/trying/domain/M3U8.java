...
import ink.kangaroo.trying.listener.DownloadListener;
import ink.kangaroo.trying.utils.JDBCUtils;
import lombok.Data;
...
import static ink.kangaroo.trying.domain.Constant.FILESEPARATOR;
...
//                    log.info("\t已完成" + finishedCount + "个，还剩" + (tsList.size() - finishedCount) + "个！");
            ...
            //删除多余的ts片段
            deleteFiles();
            JDBCUtils.update("update info set flag = 1 where Name = ?", fileName);
...
//            log.info("第{}段，url：{},filePath:{}",i,urls,dir + FILESEPARATOR + fileName + FILESEPARATOR + i + ".xy");
...
//                    e.printStackTrace();
            ...
            finishedCount++;
            log.info(urls + "下载完毕！\t已完成" + finishedCount + "个，还剩" + (tsList.size() - finishedCount) + "个！");
        });
    }
